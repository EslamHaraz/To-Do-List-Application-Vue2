  ...
  <div class="col-lg-12 flex-col">
    <form @submit.prevent class="form-grid p-3">
      <div class="form-group input-width mb-3">
        ...
        <div v-if="mangersNames.includes(this.usedName)" class="feedback-error">
          {{ $t("message.usedName") }}
        </div>
      </div>
      <div class="form-group input-width mb-3">
...
export default {
  ...
  computed: {
    mangersNames() {
      return this.$store.state.allMangersData
        .filter((ele) => {
  ...
  methods: {
    getUserData() {
      setTimeout(() => {
        this.usedName = this.$store.state.mangerData.Name;
        this.name = this.$store.state.mangerData.Name;
        this.photo = this.$store.state.mangerData.Photo;
        this.usedPhone = this.$store.state.mangerData.Phone;
        this.address = this.$store.state.mangerData.Address;
        this.type = this.$store.state.mangerData.Type;
      }, 700);
    },
    editMangerData() {
      this.v$.$validate();
      if (!this.v$.$error) {
        if (!this.mangersNames.includes(this.usedName)) {
          axios
            .put(
              `Mangers/${this.$store.state.mangerId}`,
              this.$store.state.mangerData,
              (this.$store.state.mangerData.Name = this.usedName),
              (this.$store.state.mangerData.Address = this.address),
              (this.$store.state.mangerData.Phone = this.phone),
              (this.$store.state.mangerData.Photo = this.photo),
              (this.$store.state.mangerData.Type = this.type)
            )
            .then(() => {
