...
export default {
  ...
  methods: {
    getUserData() {
      setTimeout(() => {
        this.mode = this.$store.state.mangerData.Mode;
        this.language = this.$store.state.mangerData.Language;
      }, 270);
    },
    editGenralSettings() {
      this.v$.$validate();
      if (!this.v$.$error) {
        axios
          .put(
            ...
            (this.$store.state.mangerData.Language = this.language),
            (this.$store.state.mangerData.Mode = this.mode)
          )
          .then(() => {
