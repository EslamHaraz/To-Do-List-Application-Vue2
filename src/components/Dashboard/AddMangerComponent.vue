<template>
  <div class="parent p-2">
    <h3 class="text-color text-initial mb-4">{{ $t("message.addManger") }}</h3>
    <form @submit.prevent class="form-grid">
      <div class="form-group input-width mb-3">
        ...
        <div v-if="mangersNames.includes(this.name)" class="feedback-error">
          {{ $t("message.usedName") }}
        </div>
      </div>
      <div class="form-group input-width mb-3">
        ...
        <div v-if="mangersEmails.includes(this.email)" class="feedback-error">
          {{ $t("message.usedEmail") }}
        </div>
      </div>
      <div class="form-group input-width mb-3">
      ...
      <div class="form-group input-width mb-3">
        <label
          for="permissions "
          class="w-100 d-block text-initial mb-2 text-color"
          >{{ $t("message.permissions") }}</label
        >
        <select
          :class="{
            'is-invalid': v$.permissions.$error,
          }"
          v-model="permissions"
          id="permissions"
          class="form-select select-language"
        >
          <option value="Manger">{{ $t("message.manger") }}</option>
          <option value="SuperManger">{{ $t("message.superManger") }}</option>
        </select>
        <div v-if="v$.permissions.$error" class="feedback-error">
          {{ $t("message.permissions") }} {{ $t("message.rquired") }}
        </div>
      </div>
      <div class="form-group input-width mb-3">
        <label
      ...
      <button @click="addNewManger()" class="btn btn-primary mt-3 signIn-btn">
        {{ $t("message.addManger") }}
      </button>
    </form>
  </div>
...
export default {
  ...
  computed: {
    mangersEmails() {
      return this.MangersList.map((ele) => {
        return ele.Email;
      });
    },
    mangersNames() {
  ...
  methods: {
    ...
    addNewManger() {
      ...
      if (this.password == this.confirmPassword) {
        if (
          !this.v$.$error &&
          !this.mangersEmails.includes(this.email) &&
          !this.mangersNames.includes(this.name)
        ) {
          axios
            .post(`Mangers`, {
              Name: this.name,
            ...
            .then(() => {
              ...
              setTimeout(() => {
                this.$router.push({ name: "MangersView" });
              }, 1500);
            })
            .catch(() => {
