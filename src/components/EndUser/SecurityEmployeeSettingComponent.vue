  ...
  <div class="col-lg-12 flex-col">
    <form @submit.prevent class="form-grid p-3">
      <div class="form-group input-width mb-3">
        ...
        />
        <div v-if="usedEmail == '' && v$.usedEmail.$error" class="feedback-error">
          {{ $t("message.email") }} {{ $t("message.rquired") }}
        </div>
        <div v-if="v$.usedEmail.$error && usedEmail.length > 0" class="feedback-error">
          {{ $t("message.invalidEmail") }}
        </div>
        <div
          v-if="mangersEmails.includes(this.email) && this.usedEmail !== this.email"
...
export default {
  ...
  computed: {
    mangersEmails() {
      return this.$store.state.allEmployeesData.map((ele) => {
        return ele.Email;
      });
    },
  },
  methods: {
    getUserData() {
      setTimeout(() => {
        ...
        this.usedEmail = this.$store.state.employeeData.Email;
      }, 700);
    },
    editEmployeeData() {
      this.v$.$validate();
        ...
        if (this.password == this.confirmPassword) {
          axios
            .put(
              ...
              (this.$store.state.employeeData.Email = this.usedEmail),
              (this.$store.state.employeeData.Password = this.password),
              (this.$store.state.employeeData.securityQuestion = this.securityQuestion),
              (this.$store.state.employeeData.Answer = this.answer)
            )
